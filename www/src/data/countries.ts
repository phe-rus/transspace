// ISO 3166-1 alpha-2 codes, matching what Cloudflare's request.cf.country
// returns and what data/restricted-countries.ts already keys against.
// No name column here on purpose: the display name is derived through
// Intl.DisplayNames (see countryName below), the same zero-dependency
// approach (public)/index.tsx already uses for the visitor's own country,
// so this list never needs its own i18n or drifts from the locale switcher.
...
// codes sorted by their localized display name, for a picker that reads
// naturally instead of in raw ISO order
