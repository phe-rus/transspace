...
| | `category` | text, required | validated against the shipped `GUIDE_CATEGORIES` constant (health, legal, career, life-skills), same pattern as `RESOURCE_CATEGORIES` |
...
| `/guides` | POST | `title`, `excerpt`, `category`, `bodyContent` (Tiptap JSON), `relatedResourceIds?`, `turnstileToken` | `id`, `status: "pending"` | session required, decoy sessions rejected | 401 no session, 403 decoy session or Turnstile failed, 422 unknown category or a `bodyContent`/`relatedResourceIds` validation failure (AC-9), 429 rate limited |
