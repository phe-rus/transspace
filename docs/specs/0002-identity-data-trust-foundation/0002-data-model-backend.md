...
1. Wire Cloudflare D1 and Drizzle ORM into the Transspace Worker, with a migration workflow mirroring Infra's `db:gen` and `db:studio` scripts, satisfies **AC-1**
2. Create the migration for `moderators` and `moderation_action`, plus a one off manual seed for the first two moderators: the operator has each person sign in once through the normal OAuth flow so their `user_link` row exists, looks up that row directly (for example via `wrangler d1 execute`, ordering by `created_at` on a fresh deployment, or by matching the `profile.display_name` they just set), and inserts the `moderators` rows with `granted_by` null, satisfies **AC-2**
3. Bind a Transspace owned R2 bucket in `wrangler.jsonc`; build the shared upload module (byte sniffed allowlist, SVG sanitizing, quota by listing against `storage_prefix`) and a public, unauthenticated read route for serving stored files back out, satisfies **AC-3**
4. Build the moderator grant and revoke server functions plus their REST wrappers, including the floor check and the `moderation_action` write, satisfies **AC-2**, **AC-6**
5. Build the shared pagination utility (default 20, max 50, cursor or offset required) every future list endpoint imports rather than reimplements, satisfies **AC-4**
6. Configure Cloudflare Rate Limiting Rules for write endpoints (Turnstile plus a strict limit) and read/search endpoints (a generous limit, no Turnstile), satisfies **AC-5**
7. Establish the `domains/<name>/{types,func,get-*,use-*,views,index}` server function folder convention (matching Infra's own) and a matching `src/routes/api/` shell for anything meant to be called from outside the router app, satisfies **AC-6**
